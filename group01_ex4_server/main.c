...
//Server messages:
#define SERVER_MAIN_MENU "Choose what to do next:\n 1. Play against another client\n2. Play against the server\n3. View the leaderboard\n4. Quit"
...
static DWORD ServiceThread(SOCKET *t_socket)
{
	...
	while (!Done)
	{
		...
		if (STRINGS_ARE_EQUAL(AcceptedStr, "1")) {
			if (find_other_player)
				AcceptedStr = NULL;
			RecvRes = ReceiveString(&AcceptedStr, *t_socket);
			if (RecvRes == TRNS_FAILED)
			{
			...
			write_move_to_file(AcceptedStr);

		}
		else if (STRINGS_ARE_EQUAL(AcceptedStr, "2"))
		{
			strcpy(SendStr, "3");
		}
		else if (STRINGS_ARE_EQUAL(AcceptedStr, "4"))
		{
...
//void create_message(char *msg_ptr, )
